...
const CartItem: React.FC<{ cart: ICartItem }> = ({ cart }) => {
  ...
  return (
    <div className='w-full h-auto grid-cols-1 lg:grid-cols-10 items-center border border-[#AF6900] rounded-[10px] py-5 lg:py-2 px-5 space-y-2'>
      <Image
        src={cart && cart.product.image}
        alt={cart.product.title}
        className='h-[158.841px] pointer-events-none col-span-2 flex justify-center w-full lg:justify-start'
      />
      ...
      <p className='font-georgia flex text-[#AF6900] text-[22px] lg:text-[32px] font-bold col-span-2 justify-self-center justify-center lg:justify-start'>
        {formatCurrency(cart && cart.product.pricePerBottle)}
      </p>

      <div className='flex items-center space-x-5 col-span-2 justify-self-end justify-center lg:justify-start'>
        <div
